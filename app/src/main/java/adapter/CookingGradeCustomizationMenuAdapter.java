...
public class CookingGradeCustomizationMenuAdapter extends RecyclerView.Adapter<CookingGradeCustomizationMenuAdapter.ViewHolder> {

    Activity activity;
    int select = -1;
    private ArrayList<CategoryItemPojo.ResponseData.cookingGrades> arrayList;
    private MenuActivity.ItemClickListener itemClickListener;
    StoreUserData storeUserData;


    public CookingGradeCustomizationMenuAdapter(Activity activity, ArrayList<CategoryItemPojo.ResponseData.cookingGrades> arrayList, MenuActivity.ItemClickListener itemClickListener) {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        final CategoryItemPojo.ResponseData.cookingGrades pojo = arrayList.get(position);

        if (select == position) {
            holder.binding.select.setImageResource(R.drawable.fill);
            if (pojo.price.length() > 0){
                itemClickListener.cookingGradesCustomizeTestClick(pojo.name+"="+pojo.price);
            }else {
                itemClickListener.cookingGradesCustomizeTestClick(pojo.name);
            }
        } else {
